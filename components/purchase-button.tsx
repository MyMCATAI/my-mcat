...
// MD Premium features from subscription-button.tsx
const premiumFeatures = {
  title: "MD Premium",
  price: "$50.00",
  description: "\"Oh, so NOW you want to get serious? Daily coins, premium features, the whole VIP treatment? Fine, but don't come crying when you actually have to study.\" — Kalypso",
  image: "/MDPremium.png",
  features: [
    "100 StudyCoins per day (worth $25/month value)",
    "Premium question explanations and insights",
...
export function PurchaseButton({ 
  ...
  const pricingOptions = [
    ...
    {
      title: premiumFeatures.title,
      price: premiumFeatures.price + "/month",
      description: premiumFeatures.description,
      image: premiumFeatures.image,
  ...
  return (
    <>
      ...
      >
        <DialogContent 
          className="max-w-5xl bg-[--theme-mainbox-color] backdrop-blur-sm bg-opacity-95 text-[--theme-text-color] border border-transparent z-[102] rounded-xl p-4 shadow-xl"
        ...
        >
          <DialogHeader>
            ...
            <p className="text-center text-[--theme-text-color]/70 -mt-4 mb-4">Choose how you want to get coins for premium features or subscribe for daily rewards</p>
          </DialogHeader>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 p-4">
            {pricingOptions.map((option, index) => (
              <button 
                ...
                className={`rounded-xl p-4 flex flex-col items-center transition-all duration-300 relative w-full text-left min-h-[480px]
                  ${option.type === 'subscription'
                    ? 'bg-[--theme-leaguecard-color] border-2 border-sky-300/50 hover:border-sky-300 hover:scale-[1.05] md:scale-[1.03] z-10'
                ...
                style={{ 
                  boxShadow: option.type === 'subscription'
                ...
                onMouseEnter={(e) => {
                  e.currentTarget.style.boxShadow = option.type === 'subscription'
              ...
              >
                {option.type === 'subscription' && (
                  <div className="absolute -top-3 -right-3 bg-sky-500 text-white px-3 py-1 rounded-full text-sm font-bold shadow-md transform rotate-6 z-[103]">
                    Best Value
                  </div>
                )}
                {option.productType === ProductType.COINS_100 && (
                ...
                <div className={`relative w-[110px] h-[110px] mb-4 rounded-full overflow-hidden 
                  ${option.type === 'subscription'
                      ? 'bg-gradient-to-b from-sky-100 to-white p-1'
                      : option.productType === ProductType.COINS_100
                ...
                <h3 className={`text-xl font-bold pointer-events-none transition-colors mb-2
                  ${option.type === 'subscription'
                      ? 'text-sky-600'
                      : option.productType === ProductType.COINS_100
                        ? 'text-purple-600'
                ...
                <p className={`text-2xl font-bold pointer-events-none mb-3
                  ${option.type === 'subscription'
                      ? 'text-sky-500'
                      : option.productType === ProductType.COINS_100
                        ? 'text-purple-500'
                ...
                <div className="flex-grow flex flex-col justify-between w-full">
                  <p className="text-sm text-[--theme-text-color] text-center pointer-events-none mb-4">
                    {option.description}
                  </p>
                </div>
                  ...
                  {loadingStates[option.productType] 
                    ...
                    : (
                      <div className="flex items-center justify-center">
                        <span>{option.type === 'subscription' ? 'Subscribe' : 'Purchase'}</span>

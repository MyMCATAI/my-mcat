...
function containsLatex(text: string | null): boolean {
...
function extractLatex(text: string | null): string[] {
  if (!text) return [];
  ...
  return matches.filter(match => 
    !(/^\\[nrt]$/.test(match)) // Exclude standalone \n, \r, \t
  );
}
...
async function main() {
  ...
  if (questionsWithLatex.length > 0) {
    ...
    fs.writeFileSync(
      'latex-questions-report.json', 
      JSON.stringify(questionsWithLatex, null, 2)
    );

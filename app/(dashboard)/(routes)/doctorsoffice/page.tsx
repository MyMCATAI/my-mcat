...
const DoctorsOfficePage: React.FC = () => {
    ...
    const fetchUserLevel = async () => {
      try {
        ...
        if (response.ok) {
          ...
          setUserLevel(highestLevel || 'PATIENT LEVEL');
        }
      } catch (error) {
  ...
  return (
    <div className="fixed inset-x-0 bottom-0 top-[4rem] flex bg-transparent text-[--theme-text-color] p-4">
      ...
      >
        <DialogContent className="bg-[--theme-doctorsoffice-accent] border text-[--theme-text-color] border-[--theme-border-color]">
          <DialogHeader>
            <DialogTitle className="text-white text-center">{"Welcome to Your Doctor's Office!"}</DialogTitle>
          </DialogHeader>
          <div className="py-4">
            <p className="text-center mb-4">{"Congratulations on starting your medical journey! You must purchase a room to begin your practice."}</p>
            <Button onClick={handleOpenMarketplace} className="w-full bg-[--theme-hover-color] text-[--theme-hover-text] hover:bg-opacity-80">
              Open Marketplace
            </Button>
          </div>
        </DialogContent>

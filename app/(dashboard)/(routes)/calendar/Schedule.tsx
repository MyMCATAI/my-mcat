...
const Schedule: React.FC<ScheduleProps> = ({ activities }) => {
  ...
  const getActivitiesForDate = (date: Date) => {
    return activities.filter(activity => isSameDay(new Date(activity.scheduledDate), date));
  };

  return (
    <div className="relative p-4 mt-4">
      <div className="absolute inset-0 gradientbg min-h-[880px] opacity-50 shadow-md"></div>
        ...
        <div className="grid grid-cols-2 gap-4 mb-5">
          {[1, 2].map((dayOffset) => {
            ...
            return (
              <div 
                key={dayOffset} 
                className="bg-[#5D84CE] text-white p-4 rounded-[10px] text-center relative group transition-transform duration-300 ease-in-out transform hover:scale-105 shadow-lg"
              >

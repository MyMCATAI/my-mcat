...
// Updates knowledge profiles based on user's recent responses to a test
...
export async function POST(req: Request) {
  ...
  try {
    ...
    const updatePromises = Object.entries(groupedResponses).map(
      async ([categoryId, responses]) => {
        const numCorrects = responses.filter((r) => r.isCorrect).length;
        const numIncorrects = responses.length - numCorrects;
        const conceptMastery =
          (numCorrects + 1) / (numCorrects + 1 + numIncorrects + 1);
